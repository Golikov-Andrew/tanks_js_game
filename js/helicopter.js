class HelicopterBlades extends GameObject {
    constructor(app, helicopter) {
        super(app, helicopter);
        this.x = 0
        this.y = 0
        this.a = 0
        this.speed_rotate = 0.1
    }
...
class HelicopterGun extends GameObject {
    constructor(app, helicopter) {
        super(app, helicopter);
        this.x = 50
        this.y = 10
...
class Helicopter extends GameObject {
    constructor(app, stage, x, y, a) {
        ...
        this.collision_objects = [
            new CollisionSphere(this.app, this, 20, 0, 40),
            new CollisionSphere(this.app, this, -30, 0, 35),
            new CollisionSphere(this.app, this, -110, 0, 20),
            new CollisionSphere(this.app, this, -75, 0, 20),
        ]
        this.children.blades = new HelicopterBlades(app, this)
        this.children.gun = new HelicopterGun(app, this)

        this.gun = this.children.gun
    ...
    handle_collisions() {
        ...
        for (let i = 0, c, coords; i < this.collision_objects.length; i++) {
            ...
            for (let j = 0, helicopter; j < this.app.stage.helicopters.length; j++) {
                helicopter = this.app.stage.helicopters[j]
                if (helicopter === this) continue;
                for (let k = 0, col_obj, col_obj_coords; k < helicopter.collision_objects.length; k++) {
                    col_obj = helicopter.collision_objects[k]
    ...
    draw(ctx) {
        ...
        ctx.fillRect(-120, -2, 70, 4)
        ctx.fillRect(-134, -10, 40, 6)
        ctx.closePath()
        ctx.stroke();
    ...
    fire() {
        if (this.is_fire) {
            ...
            fire_sound.play()
            let global_coords = this.children.gun.get_global_xya()
            let new_bullet = new Bullet2(
